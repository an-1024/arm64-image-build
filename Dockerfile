...
RUN set -eux; \
    ...
    install_yum() { \
        ...
        yum install -y --allowerasing --nobest ca-certificates bash coreutils findutils grep sed libcurl curl wget tar gzip xz make gcc gcc-c++ perl \
...
RUN set -eux; \
    ...
    install_yum() { \
        ...
        printf '[openeuler]\nname=openEuler 20.03 LTS\nbaseurl=https://repo.openeuler.org/openEuler-20.03-LTS/OS/aarch64/\nenabled=1\ngpgcheck=0\n' \
            > /etc/yum.repos.d/openeuler.repo; \
        yum install -y --allowerasing --nobest ca-certificates bash coreutils findutils grep sed procps-ng file binutils openssl-libs zlib pcre pcre2; \
        yum clean all; \
    }; \
    if command -v apt-get >/dev/null 2>&1; then install_apt; \
    elif command -v dnf >/dev/null 2>&1; then yum() { dnf "$@"; }; install_yum; \
